...
public class TableView extends View {
    ...
    // 坐标轴的数值
    private int mCoordinateYCount = 7;
    private String[] mCoordinateXValues=new String[]{ "日", "一", "二", "三", "四", "五", "六"};// 外界传入
    ...
    private void init() {
        ...
        // 坐标系的单位
        mBottomTextHeight = dp2px(30);// X轴底部字体的高度
...
//            if (i == 0) continue;
        ...
        for (int i = 0; i < yLineCount; i++) {
            ...
            canvas.drawLine(start.x, start.y, stop.x, stop.y, mCoordinatorPaint);
            // 绘制横坐标单位
            String drawText = String.valueOf(mCoordinateXValues[i]);
            Paint.FontMetricsInt fontMetrics = mTextPaint.getFontMetricsInt();
            float offsetY = ((fontMetrics.bottom - fontMetrics.top) / 2 + fontMetrics.bottom) / 2;
    ...
    private void drawWrap(Canvas canvas) {
        ...
        for (WaveConfigData wave : mWaves) {
            ...
            if (wave.isCoverRegion) {
                mWrapPaint.setStyle(Paint.Style.FILL_AND_STROKE);
